...
function VisitorsBox({ count }: { count: number | null }) {
  return (
    <div className="mt-3 border border-sky-700 bg-sky-700 p-2.5 text-center">
      <div className="text-[11px] font-semibold text-white/90">누적 방문수</div>
      <div className="mt-0.5 text-lg font-extrabold text-white">{count === null ? "-" : count.toLocaleString()}</div>
    </div>
  );
}
...
function AnonymousChatBox({ me }: { me: Me }) {
  ...
  const [cursor, setCursor] = useState<string | null>(null); // oldest created_at in current list
      ...
      <div className="px-3 pb-3">
        <div className="h-[220px] overflow-auto border border-slate-200 bg-white p-2 space-y-2">
          ...
          {msgs.map((m) => {
            ...
            const name = m.username ?? "익명";
            return (
              <div key={m.id} className={mine ? "flex justify-end" : "flex justify-start"}>
                <div className={"max-w-[85%] " + (mine ? "text-right" : "text-left")}>
                  <div className="text-[10px] text-slate-500 mb-0.5">{mine ? "" : name}</div>
                  <div
                    className={
                      "inline-block px-3 py-2 text-[12px] leading-snug border " +
                      (mine ? "bg-sky-50 border-sky-200 text-slate-900" : "bg-slate-50 border-slate-200 text-slate-900")
                    }
                  >
                    {m.content}
                  </div>
                ...
                {notiOpen ? (
                  <div className="absolute right-0 top-9 z-50 w-[320px] overflow-hidden border border-slate-300 bg-white shadow-lg">
                    ...
                    <div className="border-t border-slate-200 px-3 py-2">
                      <Link href="/community/free?mine=1" className="block text-[12px] font-semibold text-slate-900 hover:underline" onClick={() => setNotiOpen(false)}>
                        내가 쓴 글
                      </Link>
