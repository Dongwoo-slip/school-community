    ...
    >
      ...
      <div style={{ minWidth: 0, flex: 1 }}>
        <div className="post-row-title">
          {isAdmin && <span style={{ fontSize: '0.65rem', fontWeight: 800, background: 'rgba(8,123,99,0.10)', color: 'var(--accent-mint)', borderRadius: 2, padding: '0.05rem 0.4rem', marginRight: '0.4rem' }}>공지</span>}
          {post.title}
        </div>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.5rem' }}>
          <span style={{ fontSize: '0.65rem', fontWeight: 800, textTransform: 'uppercase', letterSpacing: '0.1em', color: 'var(--brand-light)', background: 'var(--brand-dim)', padding: '0.2rem 0.6rem', borderRadius: 2 }}>청주고등학교</span>
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: '1.25rem' }}>
          ...
          <div>
            {loading ? (
              Array.from({ length: 5 }).map((_, i) => (
                <div key={i} style={{ height: 52, background: 'var(--bg-elevated)', margin: '0.5rem', borderRadius: 4, animation: 'pulse 1.5s infinite' }} />
              ))
        ...
        {/* Meal placeholder */}
        <div style={{ background: 'var(--bg-surface)', border: '1px solid var(--border-subtle)', borderRadius: 4, padding: '1rem' }}>
          ...
          <div style={{ textAlign: 'center', padding: '1.5rem', color: 'var(--text-muted)', fontSize: '0.8rem', borderRadius: 4, background: 'var(--bg-elevated)' }}>
            급식 탭에서 확인하세요
          </div>
        </div>
      </div>
